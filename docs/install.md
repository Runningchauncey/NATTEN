...
**Newest release: `0.21.7`** |
**[Changelog](https://github.com/SHI-Labs/NATTEN/blob/main/CHANGELOG.md#0217---2026-07-26).**
...
???+ pip-install "`torch==2.13.0+cu132` | x86-64 and aarch64"

    ```python
    pip install natten==0.21.7+torch2130cu132 -f https://whl.natten.org
    ```

??? pip-install "`torch==2.13.0+cu130` | x86-64 and aarch64"

    ```python
    pip install natten==0.21.7+torch2130cu130 -f https://whl.natten.org
    ```

??? pip-install "`torch==2.13.0+cu126` | x86-64 and aarch64"

    ```python
    pip install natten==0.21.7+torch2130cu126 -f https://whl.natten.org
    ```
...
??? pip-install "`torch==2.12.0+cu132` | x86-64 and aarch64"

    ```python
    pip install natten==0.21.7+torch2120cu132 -f https://whl.natten.org
    ```

??? pip-install "`torch==2.12.0+cu130` | x86-64 and aarch64"

    ```python
    pip install natten==0.21.7+torch2120cu130 -f https://whl.natten.org
    ```

??? pip-install "`torch==2.12.0+cu126` | x86-64 and aarch64"

    ```python
    pip install natten==0.21.7+torch2120cu126 -f https://whl.natten.org
    ```
...
```python
NATTEN_CUDA_ARCH="8.9" pip install natten==0.21.7 # (1)!

NATTEN_CUDA_ARCH="9.0" pip install natten==0.21.7 # (2)!

NATTEN_CUDA_ARCH="10.0;10.3" pip install natten==0.21.7 # (3)!

NATTEN_CUDA_ARCH="8.0;8.6;9.0;10.0" pip install natten==0.21.7 # (4)!
```
...
```python
NATTEN_N_WORKERS=16 pip install natten==0.21.7 # (1)!

NATTEN_N_WORKERS=64 pip install natten==0.21.7 # (2)!
```
...
[Changelog](https://github.com/SHI-Labs/NATTEN/blob/main/CHANGELOG.md#0216---2026-04-14).
...
??? pip-install "`torch==2.12.0+cu132` | x86-64 and aarch64"
    ...
    !!! warning
        This build does not support `cp313-313t`. PyTorch's build container drops that version
        entirely in the `cuda13.2` tag.

??? pip-install "`torch==2.12.0+cu130` | x86-64 and aarch64"

    ```python
    pip install natten==0.21.6+torch2120cu130 -f https://whl.natten.org
    ```

??? pip-install "`torch==2.12.0+cu126` | x86-64 and aarch64"

    ```python
    pip install natten==0.21.6+torch2120cu126 -f https://whl.natten.org
    ```
...
??? pip-install "`torch==2.8.0+cu130` | x86-64 only"
